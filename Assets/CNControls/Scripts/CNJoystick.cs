...
public class CNJoystick : CNAbstractController
{
    ...
    protected override void OnFingerLifted()
    {
		...
		//Debug.Log("CNJoystick OnFingerLifted ");
		//playerControl.joystick = 0;
    }
    ...
    protected virtual void Update()
    {
        ...
        if (IsTouchCaptured(out currentTouch)) {
			...
			Debug.Log("CNJoystick Update " + currentTouch.deltaPosition.ToString());
		}
    }
    ...
    protected override void TweakControl(Vector2 touchPosition)
    {
            ...
            DragRadius * DragRadius)
        {
			...
			//Debug.Log("CNJoystick TweakControl " + differenceVector[1].ToString());

			// Check Joystick Action
			var moveX = differenceVector[0];
			var moveY = differenceVector[1];
			
			//Debug.Log("CNJoystick joystick (moveX, moveY) : " + moveX.ToString() + ", " + moveY.ToString());

			float maxRange = 0.5f;
			float minRange = 0.2f;


			if((moveX > -maxRange && moveX < maxRange) && moveY > maxRange) {
				playerControl.SetJoystick(1, moveX, moveY);
			} else if(moveX > maxRange && moveY > maxRange) {
				playerControl.SetJoystick(2, moveX, moveY);
			} else if(moveX > maxRange && (moveY > -maxRange && moveY < maxRange)) {
				playerControl.SetJoystick(3, moveX, moveY);
			} else if(moveX > maxRange && moveY < -maxRange) {
				playerControl.SetJoystick(4, moveX, moveY);
			} else if((moveX > -maxRange && moveX < maxRange) && moveY < -maxRange) {
				playerControl.SetJoystick(5, moveX, moveY);
			} else if(moveX < -maxRange && moveY < -maxRange) {
				playerControl.SetJoystick(6, moveX, moveY);
			} else if(moveX < -maxRange && (moveY > -maxRange && moveY < maxRange)) {
				playerControl.SetJoystick(7, moveX, moveY);
			} else if(moveX < -maxRange && moveY > maxRange) {
				playerControl.SetJoystick(8, moveX, moveY);
			}
        } else {
            // If we're inside the drag range, just place it under the finger
			_stickTransform.position = worldTouchPosition;
			
			//Debug.Log("CNJoystick joystick inside drag range");
			playerControl.SetJoystick(0);
        }
